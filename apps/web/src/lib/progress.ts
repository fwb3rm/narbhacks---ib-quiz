...
export class ProgressService {
  ...
  static getQuestionHistoryBySubcategory(
    subcategory: string
  ): QuestionHistory[] {
  ...
  static getWrongAnswersBySubcategory(subcategory: string): QuestionHistory[] {
    const subcategoryHistory =
      ProgressService.getQuestionHistoryBySubcategory(subcategory);
  ...
  static analyzeWrongAnswerPatterns(subcategory: string): WrongAnswerPattern {
    ...
    wrongAnswers.forEach((q) => {
      wrongAnswerContentCounts[q.userAnswer] =
        (wrongAnswerContentCounts[q.userAnswer] || 0) + 1;
    });

    const mostFrequentWrongAnswer =
      Object.entries(wrongAnswerContentCounts).sort(
        ([, a], [, b]) => b - a
      )[0]?.[0] || "";

    // Analyze improvement trend (simplified - could be enhanced)
    const recentWrongAnswers = wrongAnswers.filter(
      (q) =>
        new Date(q.dateAnswered) >
        new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
    );
    const olderWrongAnswers = wrongAnswers.filter(
      (q) =>
        new Date(q.dateAnswered) <=
        new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
    );
  ...
  static analyzeConceptualGaps(subcategory: string): string[] {
    ...
    wrongAnswers.forEach((q) => {
      ...
      if (wrongAnswer.includes("comparable") && correctAnswer.includes("dcf")) {
        answerPatterns["DCF vs Comparables"] =
          (answerPatterns["DCF vs Comparables"] || 0) + 1;
      }
      if (
        wrongAnswer.includes("equity") &&
        correctAnswer.includes("enterprise")
      ) {
        answerPatterns["Equity vs Enterprise Value"] =
          (answerPatterns["Equity vs Enterprise Value"] || 0) + 1;
      }
      if (wrongAnswer.includes("debt") && correctAnswer.includes("equity")) {
        answerPatterns["Debt vs Equity"] =
          (answerPatterns["Debt vs Equity"] || 0) + 1;
      }
      if (wrongAnswer.includes("revenue") && correctAnswer.includes("ebitda")) {
        answerPatterns["Revenue vs EBITDA"] =
          (answerPatterns["Revenue vs EBITDA"] || 0) + 1;
      }
      if (wrongAnswer.includes("cash") && correctAnswer.includes("accrual")) {
        answerPatterns["Cash vs Accrual"] =
          (answerPatterns["Cash vs Accrual"] || 0) + 1;
      }
      if (
        wrongAnswer.includes("operating") &&
        correctAnswer.includes("capital")
      ) {
        answerPatterns["Operating vs Capital"] =
          (answerPatterns["Operating vs Capital"] || 0) + 1;
      }
      if (wrongAnswer.includes("fifo") && correctAnswer.includes("lifo")) {
        answerPatterns["FIFO vs LIFO"] =
          (answerPatterns["FIFO vs LIFO"] || 0) + 1;
      }
      if (
        wrongAnswer.includes("wacc") &&
        correctAnswer.includes("cost of equity")
      ) {
        answerPatterns["WACC vs Cost of Equity"] =
          (answerPatterns["WACC vs Cost of Equity"] || 0) + 1;
      }
      if (
        wrongAnswer.includes("terminal") &&
        correctAnswer.includes("explicit")
      ) {
        answerPatterns["Terminal vs Explicit"] =
          (answerPatterns["Terminal vs Explicit"] || 0) + 1;
      }
      if (
        wrongAnswer.includes("synergy") &&
        correctAnswer.includes("goodwill")
      ) {
        answerPatterns["Synergies vs Goodwill"] =
          (answerPatterns["Synergies vs Goodwill"] || 0) + 1;
      }
    });
  ...
  static getPerformanceInsights(subcategory: string): PerformanceInsight {
    ...
    const currentAccuracy =
      allHistory.length > 0
        ? Math.round(
            ((allHistory.length - wrongAnswers.length) / allHistory.length) *
              100
          )
        : 0;
  ...
  static getWeakestSubcategories(limit: number = 5): PerformanceInsight[] {
    ...
    const subcategories = Array.from(
      new Set(allHistory.map((q) => q.subcategory))
    );
    ...
    const insights = subcategories
      ...
      .sort((a, b) => {
        ...
        const aWeaknessScore =
          (100 - a.currentAccuracy) * Math.min(a.totalQuestions / 5, 1); // Normalize by 5 questions
        const bWeaknessScore =
          (100 - b.currentAccuracy) * Math.min(b.totalQuestions / 5, 1);
  ...
  static getWeakestSubcategoriesWithThreshold(
    limit: number = 5,
